...
class Fun(commands.Cog):
    ...
    async def meme(self, ctx):
        reddit = asyncpraw.Reddit(
            client_id="dCkm3SQor3zow8jtYJJhQQ",
            client_secret="IJZBZmvW3QHAhowR6cNpObQc_J-UmA",
            user_agent="Fish_Bot",
        )
        subreddit = await reddit.subreddit("memes")
        submissions = []
        async for item in subreddit.hot(limit=100):
            submissions.append(item)
        submission = random.choice(submissions)
        embed = discord.Embed(title=f"{submission.title}", color=0xff4500)
    ...
    async def leaderboard(self, ctx):
        ...
        embed = discord.Embed(title = f"__Fish Bot's Global Leaderboard__", color=0x45d5d5)
        print(wordle_total)
        print(trivia_total)
        wordle_field = ""
        trivia_field = ""
