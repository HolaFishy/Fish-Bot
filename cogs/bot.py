...
class Events(commands.Cog):
    ...
    async def on_ready(self):
        ...
        embed.set_footer(text=f"{time[:16]} (CST/Central Time)", icon_url=image.avatar_url)
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith(">"):
            channel = self.client.get_channel(913504311240171590)
    ...
    async def on_guild_join(self, guild):
        if guild.name != None:
            ...
            embed = discord.Embed(title=f"{guild.name} has added Fish Bot to their server!", 
    ...
    async def on_guild_remove(self, guild):
        if guild.name != None:
            ...
            embed = discord.Embed(title=f"{guild.name} has removed Fish Bot from their server!", 
            description=f"{guild.name}'s Description: ```\n{guild.description}\n```", 
            color=0x225c9a)
            embed.set_thumbnail(url=guild.icon_url)
        ...
        try:
            if str(reaction) == "🗑️":
                ...
                if message.content.lower().startswith("yes,"):
                    await user.send("Thank you for the feedback! This helps unwanted errors get sorted out :) If Hola_Fishy#4591 sends you a friend request he is contacting you about the error!")
                    embed=discord.Embed(title="⚠️ Bug Report ⚠️", color=0xFFF100)
...
class Commands(commands.Cog):
    ...
    async def suggest(self, ctx, *, suggestion=None):
        ...
        devembed.set_thumbnail(url=ctx.author.avatar_url)
    ...
    async def support(self, ctx):
        ...
        inline=False)
        embed.add_field(name="Vote Link:", value="> https://top.gg/bot/797534062721368135/vote", inline=False)
        embed.set_footer(text="If you need to contact my developer, join the discord support server and give him a ping!", icon_url=ctx.author.avatar_url)
        await ctx.message.add_reaction("✅")
