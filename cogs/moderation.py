...
class Moderation(commands.Cog):
    ...
    async def ban(self, ctx, member:discord.Member=None, *, reason=None):
        ...
        if member == None:
            ...
            embed.set_footer(text=f"{member} was banned for: {reason}, At: {time[:16]} EST", icon_url=member.avatar_url)
            await ctx.reply(embed=embed, mention_author=False)
    ...
    async def unban(self, ctx, *, member=None):
        ...
        for ban_entry in banned_users:
            ...
            if (user.name, user.discriminator) == (member_name, member_discriminator):
                ...
                embed.set_footer(text=f"{member} was unbanned at: {time[:16]} EST", icon_url=user.avatar_url)
                await ctx.reply(embed=embed, mention_author=False)
            else:
                embed = discord.Embed(title="❌ Member not found! To unban someone you must enter their username this format: `<name>#<discriminator/numbers>`", color=0xFF0000)
                await ctx.reply(embed=embed, mention_author=False)
    ...
    async def kick(self, ctx, member: discord.Member=None, reason=None):
        ...
        if member == None:
            ...
            embed.set_footer(text=f"{member} was kicked for: {reason}, At: {time[:16]} EST", icon_url=member.avatar_url)
            await ctx.reply(embed=embed, mention_author=False)
